...
export const requestDevLogger = {
	...
	requestDidStart (requestContext) {
		...
		const avoidLog = excludeThisQueryFromLogger.some((excludedQuery) =>
			requestContext.request.query.match(excludedQuery)
		);

		if (avoidLog) {
			return;
		}
		// logger.debug(`User ${user._id} logged in successfully.`);
		...
		return {
			...
			willSendResponse (requestContext) {
				...
				if (requestContext.response.errors) {
					logger.debug(
						`Response errors (number of errors: ${requestContext.response.errors.length}):`
					);
					requestContext.response.errors.forEach((err) =>
