...
export const serviceValidations = {
     ...
     * @param {String} serviceUserId - The ID of the service user whose appointments are being requested
     */
    ensureUserCanViewServiceAppointments: async (context, serviceUserId) => {
        ...
        if (requestingUser._id.toString() !== serviceUserId && !requestingUser.isAdmin) {
            throw new ForbiddenError('You do not have permission to view these appointments');
        }
    },
     ...
     */
    listAppointmentsForServiceUser: async (serviceUserId) => {
        return await models.Appointment.find({ serviceUserId }).populate('userId').populate('serviceId');
    },
